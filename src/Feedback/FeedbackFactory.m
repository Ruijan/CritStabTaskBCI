classdef FeedbackFactory < handle
	methods(Static)
		function feedback = createFeedback(varargin)
			...
			elseif strcmp(p.Results.mode, 'ConnectedOnline')
				disp('Create Connected Feedback Online');
				feedback = ConnectedOnlineFeedback(p.Results.loop, p.Results.tobiIDSet, p.Results.system);
		...
		function valid = isValidFeedback(controllerMode)
			...
			if ~ischar(controllerMode) || (~strcmp(controllerMode,'Visual') && ...
				...
				~strcmp(controllerMode,'ConnectedOnline') && ...
